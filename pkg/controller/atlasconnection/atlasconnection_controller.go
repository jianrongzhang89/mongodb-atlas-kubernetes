...
func (r *MongoDBAtlasConnectionReconciler) Reconcile(cx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if conn.Status.ConnectionInfoRef == nil {
		...
		cmCreated, err := r.Clientset.CoreV1().ConfigMaps(req.Namespace).Create(cx, cm, metav1.CreateOptions{})
		if err != nil {
	...
	if conn.Status.CredentialsRef == nil {
		...
		secretCreated, err := r.Clientset.CoreV1().Secrets(req.Namespace).Create(cx, secret, metav1.CreateOptions{})
		if err != nil {
